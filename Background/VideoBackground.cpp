...
STDMETHODIMP CVideoBackground::CSampleGrabberCB::BufferCB( double SampleTime, BYTE * pBuffer, long BufferSize )
    {
...
//		BITMAPINFO bi;
		...
		//	if( pBuffer )
			{
//				Bitmap*  Bmp = ::new Bitmap( &bi , pBuffer );
				memcpy( &GetEditorState().GetBitmap().Info, &bih, sizeof(BITMAPINFOHEADER));
				GetEditorState().GetBitmap().Bitmap.resize( bih.biSizeImage );
				GetEditorState().GetBitmap().Bitmap.assign( pBuffer, pBuffer + bih.biSizeImage );
//				GetEditorState().SetBitmap(Bmp);
			}
...
void CVideoBackground::Draw(Graphics& gr, CRect rc) const
{
//	if( GetEditorState().GetSync().Try() )
	{
...
void CVideoBackground::Grab( CWnd * pWnd, const std::wstring& FileName)
{
	...
	if( Size != ImageSize )
	{
		...
		throw VideoSizeException(szExc );
	}
	CB.SetSize( vih->bmiHeader.biWidth, vih->bmiHeader.biHeight);
    FreeMediaType( mt );
